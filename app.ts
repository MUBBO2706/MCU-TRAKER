...
// Dynamic PWA Web App Manifest endpoint supporting custom selected App Icons
app.get("/api/manifest.json", (req, res) => {
  const icon = (req.query.icon as string) || "icon-arc-reactor";
  const validIcons = ["icon-arc-reactor", "icon-quantum-blue", "icon-vibranium-silver", "icon-infinity-gold", "icon-dark-stealth"];
  const selectedIcon = validIcons.includes(icon) ? icon : "icon-arc-reactor";

  res.setHeader("Content-Type", "application/json");
  res.json({
    name: "Nexus MCU Companion | Marvel Tracker",
    short_name: "MCU Companion",
    description: "Explore the Marvel Cinematic Universe with Nexus MCU Companion. Track movie and series watch history, explore chronological timelines, character codexes, and deep viewing analytics.",
    start_url: "/",
    display: "standalone",
    ...
    shortcuts: [
      {
        ...
        description: "MCU Multiverse Alerts and Progress",
        url: "/dashboard",
        icons: [{ src: "/shortcuts/shortcut-dashboard.png", sizes: "192x192", type: "image/png" }]
      },
      {
        name: "Movies",
        short_name: "Movies",
        description: "Browse and track MCU Movies",
        url: "/movies",
        icons: [{ src: "/shortcuts/shortcut-movies.png", sizes: "192x192", type: "image/png" }]
      },
      {
        name: "Series",
        short_name: "Series",
        description: "Track Marvel TV Series",
        url: "/series",
        icons: [{ src: "/shortcuts/shortcut-series.png", sizes: "192x192", type: "image/png" }]
      },
      {
        name: "Timeline",
        short_name: "Timeline",
        description: "MCU Story Order Timeline",
        url: "/timeline",
        icons: [{ src: "/shortcuts/shortcut-timeline.png", sizes: "192x192", type: "image/png" }]
      },
      {
        name: "Analytics",
        short_name: "Analytics",
        description: "Detailed watch time statistics",
        url: "/analytics",
        icons: [{ src: "/shortcuts/shortcut-analytics.png", sizes: "192x192", type: "image/png" }]
      }
    ]
  });
});
  ...
  app.post("/api/user/log-action", async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return res.status(401).json({ error: "Unauthorized: Missing token" });
    }
