...
export function SeriesTab({
  ...
  const getFilteredSeries = () => {
    let list = MCU_TITLES.filter((m) => m.type === 'series');
  ...
  return (
    <>
      {/* Search Input Bar */}
      <div className="relative flex items-center mb-4" id="series-search-bar">
        <Search className="absolute left-3 w-4 h-4 text-neutral-400" />
        <input
          type="text"
          placeholder="Search TV shows by cast, creator, title..."
          value={searchQuery}
        ...
        <button
          ...
          className={`absolute right-3 p-1 rounded cursor-pointer transition-colors ${showFilters ? 'text-marvel' : 'text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200'}`}
        >
      ...
      {showFilters && (
        <div className="p-4 bg-neutral-950 rounded-2xl border border-neutral-800 grid grid-cols-2 gap-3.5 animate-fadeIn relative z-30 mb-4" id="series-filter-drawer">
      ...
      {/* TV Specific list */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-6 gap-y-4" id="series-grid">
                ...
                <div className="flex-grow flex flex-col justify-between py-0.5 min-w-0">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-neutral-400">
                      {m.releaseYear} • TV Series • {m.runtimeMinutes} min total • Phase {m.phase}
                    </p>
